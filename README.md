...
- **A model's account of its own process is not evidence.** Asked "walk me
  through your procedure", a production model described a date filter its tool
  did not have, JSON output it did not produce, "top 25 hits" when it had
  requested 10, and link verification it never performed — then blamed an index
  lag that did not exist. The post-mortem is generated by the same machinery as
  the answer and is *more* convincing, because it is structured and
  self-critical. Grade the trace; do not trust the confession.
